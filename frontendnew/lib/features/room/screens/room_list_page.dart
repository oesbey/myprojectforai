...
class _RoomListPageState extends State<RoomListPage> {
  ...
  Widget _buildTopBar() {
    // ELI5: Sabit (değişmeyen) kısımlara const ekledik ki uygulama hızlı çalışsın
    return const Padding(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 8),
          Text("İstanbul", style: TextStyle(fontSize: 16, color: Colors.grey)),
          Spacer(),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            // ELI5: OPACITY HATASI BURADAN GİDERİLDİ! (.withValues kullanıldı)
            color:
                room.frameColor?.withValues(alpha: 0.5) ?? Colors.transparent,
            width: 2,
          ),
          boxShadow: [
            BoxShadow(
              // ELI5: OPACITY HATASI BURADAN GİDERİLDİ!
              color: (room.frameColor ?? Colors.black).withValues(alpha: 0.05),
              blurRadius: 10,
